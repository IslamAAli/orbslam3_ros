...
public:
    ...
    ProbabilityMapping(ORB_SLAM3::Map *pMap);

    /* * \brief void SemiDenseLoop(ORB_SLAM3::KeyFrame kf, depthHo**, std::vector<depthHo>*): return results of epipolar search (depth hypotheses) */
    void SemiDenseRecon(ORB_SLAM3::KeyFrame* kf);
    ...
    void EpipolarSearch(ORB_SLAM3::KeyFrame *kf1, ORB_SLAM3::KeyFrame *kf2, const int x, const int y, float pixel, float min_depth, float max_depth, depthHo *dh, cv::Mat F12, float &best_u, float &best_v,float th_pi,float rot);
    void GetSearchRange(float& umin, float& umax, int px, int py, float mind, float maxd, ORB_SLAM3::KeyFrame* kf, ORB_SLAM3::KeyFrame* kf2);
...
private:
    void ComputeInvDepthHypothesis(ORB_SLAM3::KeyFrame* kf, ORB_SLAM3::KeyFrame *kf2, float ustar, float ustar_var, float a, float b, float c, depthHo *dh, int x, int y);
